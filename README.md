...
8. **增加簡碼提示：** 用長碼上字後，若該字有更短的編碼，會跳出系統通知顯示「字 → 簡碼」，幫助逐步學會簡碼
    1. 預設開啟。不需要的人，打開`liur.custom.yaml`，將`commit_hint`的`reset: 1`改為`reset: 0`，再重新部署即可
    2. 若通知沒出現，請至`系統設定 → 通知`確認`工序指令編寫程式`的通知權限
    3. 僅提示單字，且輸入碼需2碼以上才會提示，避免通知洗版
